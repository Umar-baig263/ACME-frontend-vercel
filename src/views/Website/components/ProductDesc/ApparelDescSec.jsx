...
const ApparelDescSec = ({ product }) => {
  ...
  const fallbackPrice = 181.95;
  const unitPrice = Number(product?.price) || fallbackPrice;
  const totalPrice = (unitPrice * items).toFixed(2);
  const itemPayload = {
    ...product,
    price: unitPrice,
    priceWas: Number(product?.priceWas) || 39.99,
    selectSize,
              ...
              <div className="flex flex-wrap gap-2">
                {['M', 'L', 'XL', 'XXL'].map((size) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-md border ${selectSize === size ? 'border-red-600 bg-red-50' : 'border-gray-300 bg-white'} text-sm`}
                  >
                    {size}
                  </button>
                ))}
