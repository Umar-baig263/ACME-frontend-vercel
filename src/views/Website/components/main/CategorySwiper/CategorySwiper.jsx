...
const CategorySwiper = ({ data, activeCategory, onCategoryClick, paddingClass }) => {

  return (
    <div className={`md:mt-10 mt-5 ${paddingClass || 'lg:px-26 md:px-10 px-5'} overflow-hidden`}>
      <div className="">
        <Swiper
          spaceBetween={0}
          //   slidesPerView={1}
        ...
        >
          {data?.map((d, i) => {
            const isActive = activeCategory === d.key || activeCategory === d.name;
            return (
              <SwiperSlide key={i} className="py-4">
                <div 
                  ...
                  onClick={() => onCategoryClick && onCategoryClick(d.key || d.name)}
                >
                  {/* Image Container */}
                  ...
                  <div className="relative flex flex-col items-center">
                    <span className={`text-sm md:text-[15px] text-center transition-colors duration-300 mb-1 ${
                      isActive ? "text-black font-bold" : "text-gray-800 font-medium group-hover:text-black"
                    }`}>
                      {d.name}
                    </span>
                    <div className={`h-[3px] rounded-full bg-[#C6131B] transition-all duration-500 ease-out ${
